...
& (Join-Path $tarJdkBin 'jpackage.exe') --type app-image --input launcher/build/libs --dest $tarDist --name 'Tar Client' --main-jar tar-launcher.jar --main-class dev.tarclient.launcher.TarLauncher --runtime-image $tarRuntime --app-version 0.1.0 --vendor 'Tarre Industries' --java-options '-Dfile.encoding=UTF-8'
...
Compress-Archive -LiteralPath (Join-Path $tarDist 'Tar Client') -DestinationPath (Join-Path $tarDist 'TarClient-0.1.0-Windows.zip')
$tarHash = Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $tarDist 'TarClient-0.1.0-Windows.zip')
Set-Content -LiteralPath (Join-Path $tarDist 'SHA256SUMS.txt') -Value ($tarHash.Hash.ToLowerInvariant() + '  TarClient-0.1.0-Windows.zip') -Encoding ascii
